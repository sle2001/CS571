...
I used size, which is an element of design, by changing the size of the featured item to make it stand out more and grab the user's attention. I used the idea of value by changing the color of the welcome message on top to a more subdued color, so that it blends in with the background, and makes the intensity of the featured item more intense. Using the design principles of movement, by having different font sizes and using a sans sarief font, the user will be paying attention to the larger fonts then to the smaller fonts.
...
![Ideating Visual Design of Mart](_figures/visual_design.png)
